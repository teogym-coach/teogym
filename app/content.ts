...
export const site = {
  ...
  keywords: ['테오짐 PT 청라점', '청라 PT', '청라 개인 PT', '청라 퍼스널트레이닝', '청라 트레이너', '청라 헬스장', '청라 체형교정', '청라 다이어트 PT', '청라 벌크업 PT', '인천 청라 PT', '운동기록 PT', '대표 직접 수업', '움직임 평가', '맞춤형 PT'],
  openGraphImage: {
  ...
  // 운동기록 페이지는 유지하되 상단 메뉴에서는 노출하지 않습니다(푸터/본문 링크로만 접근).
  // 가격 페이지는 홈/메뉴/푸터에서 완전히 비노출 처리(대표 지시) — SEO/타 페이지 내부링크 용도로만 파일 유지.
  nav: [
...
export const footerNav = [
  ...
  { heading: '관리 시스템', items: [['PT 시스템 안내', '/system/'], ['운동기록 관리', '/records/'], ['회원 전용 앱', '/app/']] },
  { heading: '안내', items: [['대표 소개', '/about/'], ['후기와 사례', '/cases/'], ['오시는 길', '/location/']] },
] as const;
...
export const pageMeta = {
  ...
  records: {
    title: '운동기록 기반 PT | 기록이 다음 수업을 만드는 테오짐',
    description: '테오짐 PT 청라점이 수업 내용과 회원 피드백을 기록하고, 이전 운동과 비교해 다음 수업을 설계하는 과정을 소개합니다.',
    path: '/records/',
  },
  app: {
  ...
  system: {
    title: '테오짐 PT 시스템 | 상담부터 운동기록·다음 수업 설계까지',
    description: '대표 직접 상담, 움직임 평가, 목표 설정, 운동기록, 회원 피드백을 바탕으로 다음 수업을 설계하는 테오짐 PT 청라점의 관리 시스템입니다.',
    path: '/system/',
  },
};
...
// Record & Care 섹션 캐러셀용 브랜드 컨셉 목업 (실제 앱/관리자 화면 아님).
// 1086×1448(3:4) 원본을 scripts/make-mockups.mjs로 AVIF(width 640) 변환해 사용합니다.
export const mockups = [
  { src: '/images/mockups/workout-record.avif', alt: '운동기록 관리 컨셉 이미지 - 종목, 세트, 중량 기록 화면 목업', width: 640, height: 853 },
  { src: '/images/mockups/progress-analysis.avif', alt: '변화 분석 컨셉 이미지 - 체중과 신체 변화 그래프 화면 목업', width: 640, height: 853 },
  { src: '/images/mockups/member-management.avif', alt: '회원 관리 컨셉 이미지 - 회원별 관리 현황 화면 목업', width: 640, height: 853 },
] as const;
...
export const faqs = {
  home: [
    ...
    { q: '대표가 직접 수업하나요?', a: '네. 모든 PT 수업은 대표가 직접 진행하며, 회원의 운동 기록과 몸 상태를 바탕으로 다음 수업까지 직접 관리합니다.' },
    { q: '예약은 어떻게 하나요?', a: '네이버 예약, 네이버 톡톡, 전화(0507-1373-7578)로 예약할 수 있습니다. 예약제로 운영하므로 방문 전 시간을 확인해주세요.' },
    { q: '운동을 오래 쉬었는데 괜찮나요?', a: '네, 괜찮습니다. 오래 쉬었던 분일수록 첫 평가에서 현재 체력과 가동성을 더 꼼꼼히 확인하고, 무리 없는 강도부터 기록을 남기며 단계적으로 올립니다.' },
    { q: '체형교정도 가능한가요?', a: '네, 체형교정 PT를 별도로 운영합니다. 자세와 움직임, 근력 불균형을 먼저 확인한 뒤 몸 상태에 맞는 운동을 지도합니다.' },
    { q: '식단 관리도 해주시나요?', a: '네. 운동기록, 체중 변화, 생활패턴을 함께 보면서 무리한 제한 없이 유지 가능한 식사 방향을 피드백합니다.' },
    { q: 'PT는 몇 회 정도 받아야 하나요?', a: '목표와 몸 상태에 따라 필요한 횟수가 달라 상담에서 함께 확인합니다. 첫 상담 후 목적에 맞는 수업 구성을 안내드립니다.' },
  ],
  posture: [
  ...
  system: [
    { q: '첫 상담에서는 무엇을 확인하나요?', a: '운동 목표, 운동 경험, 가능한 운동 빈도, 생활 패턴, 기존에 불편했던 부위, 선호하는 운동 방식을 확인합니다. 의학적 진단이 아니라 운동 방향을 정하기 위한 상담입니다.' },
    { q: '운동을 처음 해도 시작할 수 있나요?', a: '네, 가능합니다. 상담과 움직임 평가에서 현재 체력과 가동성을 먼저 확인한 뒤, 몸 상태에 맞는 강도로 시작합니다.' },
    { q: '매 수업마다 운동이 달라지나요?', a: '고정된 루틴을 그대로 반복하지 않습니다. 지난 수업의 운동기록과 몸 상태를 확인해 종목, 강도, 순서를 조정하기 때문에 수업마다 조금씩 달라집니다.' },
    { q: '운동기록은 회원도 확인할 수 있나요?', a: '네. 수업에서 남긴 운동기록과 체중 변화, 다음 수업 일정을 회원 전용 앱에서 언제든 확인할 수 있습니다.' },
    { q: '다이어트와 근력 운동을 함께 진행할 수 있나요?', a: '네, 가능합니다. 운동기록과 체중 변화, 생활패턴을 함께 관리하며 감량과 근력 향상을 같은 흐름 안에서 진행합니다.' },
    { q: '몸이 불편한 날에도 수업을 진행하나요?', a: '먼저 당일 몸 상태와 불편한 동작을 확인합니다. 운동이 가능한 경우에는 운동 부위와 강도를 조절하거나 컨디셔닝 중심으로 진행합니다. 통증이 심하거나 원인을 확인해야 하는 상태라면 운동보다 의료기관 진료를 먼저 권장합니다.' },
    { q: '대표가 모든 수업을 직접 진행하나요?', a: '네. 상담부터 수업까지 대표가 직접 진행하며, 담당자가 중간에 바뀌지 않습니다.' },
    { q: '상담 전에 준비할 것이 있나요?', a: '특별히 준비할 것은 없습니다. 편한 복장으로 방문해주시면 되고, 최근 운동 경험이나 불편했던 부위가 있다면 상담에서 말씀해주시면 도움이 됩니다.' },
  ],
} as const;
